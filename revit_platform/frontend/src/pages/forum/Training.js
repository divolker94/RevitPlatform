import React from 'react';
import '../Forum.css';

function Training() {
    const trainingArticles = [
        {
            id: 7,
            title: 'Основы работы с Revit для начинающих',
            date: '2024-02-05',
            author: 'Преподаватель BIM',
            preview: 'Базовый курс для тех, кто только начинает работать с Revit...',
            image: '/images/blog/basics.jpg',
            category: 'Обучение'
        },
        {
            id: 8,
            title: 'Продвинутые техники моделирования',
            date: '2024-01-30',
            author: 'BIM-эксперт',
            preview: 'Углубленный курс по продвинутым методам моделирования в Revit...',
            image: '/images/blog/advanced.jpg',
            category: 'Обучение'
        }
    ];

    return (
        <div className="forum-container">
            <div className="forum-header">
                <h1>Обучение Revit</h1>
                <p>Учебные материалы и курсы</p>
            </div>

            <div className="forum-grid">
                {trainingArticles.map(article => (
                    <article key={article.id} className="forum-card">
                        <div className="forum-card-image">
                            <img src={article.image} alt={article.title} />
                            <span className="forum-category">{article.category}</span>
                        </div>
                        <div className="forum-card-content">
                            <h2>{article.title}</h2>
                            <p className="forum-preview">{article.preview}</p>
                            <div className="forum-meta">
                                <span className="forum-author">
                                    <i className="fas fa-user"></i> {article.author}
                                </span>
                                <span className="forum-date">
                                    <i className="fas fa-calendar"></i> {new Date(article.date).toLocaleDateString()}
                                </span>
                            </div>
                            <button className="read-more">Перейти к обсуждению</button>
                        </div>
                    </article>
                ))}
            </div>
        </div>
    );
}

export default Training;