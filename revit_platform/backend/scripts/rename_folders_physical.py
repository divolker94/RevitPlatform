#!/usr/bin/env python
"""
Скрипт для физического переименования папок BIM семейств в простые 5-значные названия
"""
...
def rename_folders_physical():
    print("🔄 ФИЗИЧЕСКОЕ ПЕРЕИМЕНОВАНИЕ ПАПОК BIM СЕМЕЙСТВ")
    print("=" * 50)
    ...
    # Сначала посмотрим, какие папки уже существуют
    existing_folders = []
    try:
    ...
    print(f"📂 Найдено папок в файловой системе: {len(existing_folders)}")
    ...
    for i, family in enumerate(families, 1):
        ...
        # Ищем папку по external_id или по названию
        old_folder_name = None
        for folder in existing_folders:
            if folder.startswith(str(family.external_id)) or folder.startswith(family.name[:10]):
        ...
        if old_folder_name:
            ...
            print(f"   {i:2d}. {old_folder_name[:30]}... → {new_folder_name}")
        else:
            print(f"   {i:2d}. ❌ Не найдена папка для: {family.name[:30]}...")
    
    print(f"\n🔄 Начинаем физическое переименование...")
    ...
    for old_name, info in folder_mapping.items():
        ...
        if os.path.exists(old_path):
            try:
                # Проверяем, не существует ли уже новая папка
                if os.path.exists(new_path):
                    print(f"   ⚠️  Папка {info['new_name']} уже существует, удаляем старую")
                    shutil.rmtree(new_path)
                
                # Переименовываем папку
                shutil.move(old_path, new_path)
                print(f"   ✅ {old_name[:30]}... → {info['new_name']}")
                ...
                print(f"   ❌ Ошибка при переименовании {old_name[:30]}...: {e}")
        else:
            print(f"   ⚠️  Папка не существует: {old_name}")
    
    print(f"\n🏁 Физическое переименование завершено!")
    print(f"📁 Новые папки:")
