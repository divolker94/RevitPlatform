# revit_platform/parser/load_to_postgres_fixed.py
import psycopg2
import csv
import json
...
def fix_catalog_items(catalog_items_str):
    ...
    # Разбиваем строку по ; и очищаем каждый элемент
    items = [item.strip() for item in catalog_items_str.split(';') if item.strip()]
    return items

def load_data():
    """Загрузка данных в PostgreSQL через Python с исправлениями"""
    ...
    try:
        ...
        with open('families_bimfamily_simple.csv', 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    ...
                    catalog_items = fix_catalog_items(row['catalog_items'])
                    ...
                    """, (
                        ...
                        row['technical_specs'], row['basic_specs'], catalog_items,
                    ...
                    # Продолжаем с следующим семейством
                    continue
        
        print(f"✅ Семейства загружены: {families_count}")
        
        # Коммитим изменения для семейств
        conn.commit()
        print("✅ Изменения для семейств зафиксированы")
        ...
        # Коммитим изменения для изображений
        conn.commit()
